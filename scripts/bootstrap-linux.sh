...
if ! configured_hostname="$(
  "${nix_cmd[@]}" eval --raw \
    "${FLAKE_REF}#nixosConfigurations.${configured_host}.config.networking.hostName"
)"; then
  die "no NixOS configuration named ${configured_host}"
fi
configured_system="$(
  "${nix_cmd[@]}" eval --raw \
    "${FLAKE_REF}#nixosConfigurations.${configured_host}.config.nixpkgs.hostPlatform.system"
)"
configured_user="$(
  "${nix_cmd[@]}" eval --raw \
    "${FLAKE_REF}#nixosConfigurations.${configured_host}.config.home-manager.users" \
...
"${nix_cmd[@]}" eval --raw \
  "${FLAKE_REF}#nixosConfigurations.${configured_host}.config.system.build.toplevel.drvPath" \
  >/dev/null

log "Building configuration ${configured_host}"
nixos-rebuild build \
  --option experimental-features "${FLAKE_FEATURES}" \
  --flake "${FLAKE_REF}#${configured_host}"
